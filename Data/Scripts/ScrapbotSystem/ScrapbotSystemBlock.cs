namespace Okp.ScrapbotSystem
{
   ...
   public class ScrapbotSystemBlock : MyGameLogicComponent
   {
      ...
      public const float WELDING_GRINDING_MULTIPLIER_MIN = 100f;
      public const float WELDING_GRINDING_MULTIPLIER_MAX = 1000f;

      public const float WELDER_REQUIRED_ELECTRIC_POWER_STANDBY_DEFAULT = 1f; //20W
      public const float WELDER_REQUIRED_ELECTRIC_POWER_WELDING_DEFAULT = 1f; //2kW
      public const float WELDER_REQUIRED_ELECTRIC_POWER_GRINDING_DEFAULT = 1f; //1.5kW
      public const float WELDER_REQUIRED_ELECTRIC_POWER_TRANSPORT_DEFAULT = 1f; //10kW
      public const float WELDER_TRANSPORTSPEED_METER_PER_SECOND_DEFAULT = 2000f;
      public const float WELDER_TRANSPORTVOLUME_DIVISOR = 0.001f;
      public const float WELDER_TRANSPORTVOLUME_MAX_MULTIPLIER = 800f;
      public const float WELDER_AMOUNT_PER_SECOND = 20f;
            ...
            if (MyAPIGateway.Session.IsServer)
            {
               ...
               if (!fast)
               {
                  if ((State.Ready != _PowerReady || State.Welding != _PowerWelding || State.Grinding != _PowerGrinding || State.Transporting != _PowerTransporting) &&
                      MyAPIGateway.Session.ElapsedPlayTime.Subtract(_UpdatePowerSinkLast).TotalSeconds > 2)
                  {
